...
export default function TransactionTable() {
  ...
  const [targetTransaction, setTargetTransaction] = useAtom(
    targetTransactionAtom,
  );
  ...
  const fetchTransactions = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/data/fetch?userName=${encodeURIComponent(userName)}`,
        {
          method: "GET",
          headers: {
      ...
      if (response.ok) {
        ...
        if (fetchData.success) {
          ...
          console.log(
            `Loaded ${fetchData.data?.length || 0} transactions for ${userName}`,
          );
        } else {
      ...
      {showEditForm && (
        <EditTransactionForm
          ...
          onSuccess={(updatedTransaction) => {
            console.log(
              "EditTransactionForm success callback:",
              updatedTransaction,
            );
            // 編集成功時の処理をここに追加
